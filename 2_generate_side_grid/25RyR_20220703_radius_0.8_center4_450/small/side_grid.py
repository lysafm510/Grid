...
def main(SMALL_COORDINATES_X, SMALL_COORDINATES_Y):
    global boungridts, bounpochs
    global np, nboun, sidetype, nbs, bounarc, px, py, pstep, bx, by, bs
    global gx, gy, gcx, gcy, gcr, x, y, xce, yce
    global bgnp, bgne, bgstep, bgpx, bgpy, bgcx, bgcy, bgcr, bgnod, bgnoe
    ...
    sidetype = SIDE_TYPE
    boungridt_file = open(boundary_gridt_file, "a")
    bounpoch_file = open(boundary_npoch_file, "a")
    # sidetype=1 离散形式的边界
    if sidetype == 1:
        filename = 'boun3.dat'
        loadboun(filename)
    ...
    # sidetype=2 单段连续边界，函数形式，须改动
        ...
        arc = 2 * pai * rad  # 小圆周长
        arcscatter(x0, y0, x1, y1, arc)
        COUNT = savedata(4, boungridt_file, bounpoch_file)
